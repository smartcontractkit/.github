...
import { getAllRoots, matchModule, matchModules } from "../path-ops";
...
describe("getAllRoots", () => {
  ...
  test("should find all go.mod files in a directory", async () => {
    ...
    const result = await getAllRoots("go.mod", "./");
    expect(result).toEqual([
      "path/to/md/file",
  ...
  test("should return an empty array if no root files are found", async () => {
    globMock.create = vi.fn().mockResolvedValue({
      glob: vi.fn().mockResolvedValue([]),
    });

    const result = await getAllRoots("package.json", "./test/empty");
    expect(result).toEqual([]);
  });
});
