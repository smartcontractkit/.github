...
def main():
    ...
    if not matching_files:
        if post_comment:
            skip_md = [
                ...
                "",
                f"*Generated by PR Quality Check* 🤖 (based on commit {head_sha_short})",
            ]
    ...
    for fpath in matching_files:
        ...
        if errors_in_file == 0 and "SUMMARY_JSON:" not in resp:
            errors_in_file = resp.count("❌")
        if warnings_in_file == 0 and "SUMMARY_JSON:" not in resp:
            warnings_in_file = resp.count("⚠️")
    ...
    if (total_errors + total_warnings) == 0:
        results_md.append("✅ **All checks passed!** Your code meets the quality standards.")
    else:
        if total_errors > 0:
            results_md.append("❌ **Please address the issues above before merging.**")
        else:
            results_md.append("⚠️ **Some warnings found.** Consider addressing them.")
    results_md.append("")
    results_md.append("---")
    results_md.append("<!-- pr-quality-check -->")
    results_md.append(f"*Generated by PR Quality Check* 🤖 - Comment updated with every commit (last refresh: {head_sha_short}) ")
