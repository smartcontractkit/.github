...
| `docker-build-cache-from` | no           | GHA cache scoped to OS/arch                           | **DEPRECATED: Use `cache-mode` instead!** Override the cache source. Effective only when cache restore is enabled via `cache-mode` (e.g. `auto` on PRs, `read-write`, or `read-only`).                                                                                      |
| `docker-build-cache-to`   | no           | GHA cache scoped to OS/arch                           | **DEPRECATED: Use `cache-mode` instead!** Override the cache destination. Effective only when cache save is enabled via `cache-mode` (e.g. `auto` on push/schedule, `read-write`, or `write-only`).                                                                         |
