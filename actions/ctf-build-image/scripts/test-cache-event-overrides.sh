...
echo "===== Testing ctf-build-image cache event overrides (Option 1) ====="
...
echo "Test 5: Explicit legacy input override for save-cache (docker-save-cache=true on pull_request)"
RES=$("$RESOLVE_SCRIPT" "auto" "true" "" "pull_request")
RES_SAVE=$(parse_val "$RES" "save-cache")
RES_RESTORE=$(parse_val "$RES" "restore-cache")
[ "$RES_SAVE" = "true" ] || (echo "FAIL: expected save=true, got $RES_SAVE" && exit 1)
[ "$RES_RESTORE" = "true" ] || (echo "FAIL: expected restore=true, got $RES_RESTORE" && exit 1)
...
echo "Test 6: cache-mode=read-only on push event (save=false, restore=true)"
