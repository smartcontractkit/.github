 ...
 *
 * Configurable defaults (allowed_authors, max_attempts, skip_labels, etc.)
 * live in workflow-config.ts as DEFAULT_MERGE_CONFLICT_CONFIG -- the single
 * source of truth.  Helper functions here accept optional overrides so
 * callers can inject values loaded from medic.yml.
 */

import { DEFAULT_MERGE_CONFLICT_CONFIG } from './workflow-config.js';

// ── Internal constants (not configurable via medic.yml) ─────────────────────
