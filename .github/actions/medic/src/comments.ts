...
import { COMMENT_MARKER, MEDIC_LOGO_SIZE, MEDIC_LOGO_URL, PRICING, RETRY_COMMENT_MARKER } from './config.js';
import { DEFAULT_MERGE_CONFLICT_CONFIG } from './workflow-config.js';
import type { CommentParams, OctokitClient, RetryCommentParams, RetryStatus, TokenUsage } from './types.js';
...
/**
 * Format status emoji for retry results
 */
...
/**
 * Format the comment body for workflow retry status updates
 */
...
|----------|--------|---------|
${params.results.filter(r => r.status !== 'no_runs').map(r => `| ${r.workflow} | ${formatRetryStatus(r.status)} | ${r.attempt}/${params.maxAttempts} |`).join('\n')}

`;

  // Show analysis reasoning and token usage for any results that have it
...
/**
 * Upsert a retry comment on a PR (update existing or create new)
 * Uses a separate marker from merge conflict comments
 */
export async function upsertRetryComment(
  ...
  try {
    // Find existing retry comment with marker
    const { data: comments } = await octokit.rest.issues.listComments({
      owner,
      repo,
      issue_number: prNumber
