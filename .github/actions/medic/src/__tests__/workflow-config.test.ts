...
import { parseConfig, parseMedicConfig, isWorkflowRetryable } from '../workflow-config.js';
...
# https://github.com/smartcontractkit/medic-test

workflow_retry:
  # Maximum number of retry attempts per workflow run
  max_attempts: 3
  
  # Workflows eligible for automatic retry (use workflow file names)
  # Only workflows listed here will be retried by /medic retry
  retryable:
    - "flaky-ci-test.yml"
    # Future: add real workflows like:
    # - "ci-core.yml"
    # - "integration-tests.yml"
`;
            const config = parseConfig(yaml);
