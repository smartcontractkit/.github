...
const DEFAULT_WORKFLOW_RETRY_CONFIG: WorkflowRetryConfig = {
  max_attempts: 3,
  retryable: [],
};
...
const mergeConflictSchema = z.object({
  allowed_authors: stringArray.default(
    DEFAULT_MERGE_CONFLICT_CONFIG.allowed_authors,
  ),
...
export function loadMedicConfigFromFile(
  filePath: string = ".github/medic.yml",
): MedicConfig {
  try {
...
export function isWorkflowRetryable(
  workflowFile: string,
